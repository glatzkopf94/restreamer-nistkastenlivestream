...
function getIngestRTSPStability(inputs = [], reconnect = false) {
	const profile = inputs.reduce((current, input) => {
		const candidate = input.livechasing?.rtspStability;
		if (candidate === 'strict') return 'strict';
		if (candidate === 'stable' && current === 'standard') return 'stable';
		return current;
	}, 'standard');

	return {
		profile,
		decoderErrorMode: profile === 'strict' ? 'explode' : profile === 'stable' ? 'careful' : 'ignore_err',
		exitOnError: profile === 'strict',
		reconnect: profile === 'strict' ? true : reconnect,
	};
}

function getHLSSegmentCleanupAge(control) {
	const listSize = parseInt(control.hls.listSize, 10);
	if (control.hls.dvr?.enabled || listSize <= 0) return 0;
	return parseInt(control.hls.segmentDuration, 10) * (listSize + 6);
}

function getPlayerAspectRatio(metadata = {}, enabled = true) {
	if (!enabled) return '16:9';

	const streams = Array.isArray(metadata.streams) ? metadata.streams : [];
	const video = streams.find((stream) => stream.type === 'video' && Number(stream.width) > 0 && Number(stream.height) > 0);
...
function isDVRChannelId(value) {
	return typeof value === 'string' && /^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/.test(value);
}

function countUniqueHLSViewers(activeSessions = {}) {
	const viewers = new Set();
	const sessions = Array.isArray(activeSessions.hls) ? activeSessions.hls : [];

	for (const session of sessions) {
		if (!session || Number(session.bandwidth_tx_kbit) <= 0) continue;

		const extra = typeof session.extra === 'string' ? session.extra.trim() : '';
		const viewer = extra.match(/^\[viewer:([A-Za-z0-9_-]{16,64})\](?:\s|$)/);
		if (viewer) {
			viewers.add(`viewer:${viewer[1]}`);
		} else if (extra) {
			// Older and third-party players have no browser token. Core's HLS
			// middleware still supplies an anonymized IP plus user agent, which
			// deduplicates multiple channels opened by the same client.
			viewers.add(`client:${extra}`);
		} else if (session.id) {
			viewers.add(`session:${session.id}`);
		}
	}
...
class Restreamer {
	...
	GetPublicIframeCode(channelid, aspectRatio = '16:9') {
		...
		const height = ratio === '4:3' ? 480 : 360;
		const cssRatio = ratio === '4:3' ? '4 / 3' : '16 / 9';

		return `<iframe src="${address}/${channel.channelid}.html" width="640" height="${height}" style="width:100%;max-width:640px;height:auto;aspect-ratio:${cssRatio};" frameborder="no" scrolling="no" allowfullscreen="true"></iframe>`;
	}

	GetPlayerAspectRatio(metadata, enabled = true) {
		return getPlayerAspectRatio(metadata, enabled);
	}
	...
	async _waitForIngestStopped(channelid) {
		for (let attempt = 0; attempt < 60; attempt++) {
			const progress = await this.GetIngestProgress(channelid);
			if (!progress.valid || (progress.order !== 'start' && ['disconnected', 'error'].includes(progress.state))) {
				return true;
			}
	...
	async _requestDVRPurge(channelIds) {
		const requestId = uuidv4();
		const requestPath = `/livechasing-control/requests/${requestId}.json`;
		const responsePath = `/livechasing-control/responses/${requestId}.json`;
		...
		try {
			for (let attempt = 0; attempt < 40; attempt++) {
				const [, existsError] = await this._call(this.api.DataHasFile, responsePath);
				if (existsError === null) {
					const [rawResponse, responseError] = await this._call(this.api.DataGetFile, responsePath);
					if (responseError !== null) return { ok: false, error: 'response-failed' };
					try {
				...
				if (existsError.code !== 404) return { ok: false, error: 'response-failed' };
	...
	async PurgeDVRChannels(channelIds) {
		channelIds = Array.from(new Set(channelIds)).filter((channelId) => isDVRChannelId(channelId) && this.GetChannel(channelId) !== null);
		if (channelIds.length === 0) {
			return { success: true, channelCount: 0, deletedChannelCount: 0, deletedFiles: 0, freedBytes: 0, restartedChannels: 0 };
		}
		...
		try {
			for (const channelId of channelIds) {
				const progress = await this.GetIngestProgress(channelId);
				if (progress.order !== 'start') continue;
				if ((await this.StopIngest(channelId)) === false) {
					preparationError = true;
					break;
				}
				stoppedChannels.push(channelId);
			}

			if (!preparationError) {
				for (const channelId of stoppedChannels) {
					if ((await this._waitForIngestStopped(channelId)) === false) {
						preparationError = true;
						break;
					}
				}
		...
		if (!preparationError) {
			try {
				response = await this._requestDVRPurge(channelIds);
			} catch (error) {
		...
		let restartedChannels = 0;
		for (const channelId of stoppedChannels) {
			let restarted = false;
			try {
				restarted = await this.StartIngest(channelId);
			} catch (error) {
				restarted = false;
			}
			if (restarted === false) {
		...
		if (preparationError || response === null) {
			response = { ok: false, error: 'prepare-failed' };
		}

		return {
			...response,
			success: response.ok === true && restartFailed === false,
			restartedChannels,
			restartFailed,
		};
	}

	async PurgeDVR(channelid) {
		return await this.PurgeDVRChannels([channelid]);
	}

	async PurgeAllDVR() {
		const channelIds = [];
		for (const channel of this.ListChannels()) {
			const metadata = await this.GetIngestMetadata(channel.channelid);
			if (metadata.control?.hls?.dvr?.enabled === true) {
				channelIds.push(channel.channelid);
			}
		}
		return await this.PurgeDVRChannels(channelIds);
	}
	...
	async UpsertIngest(channelid, global, inputs, outputs, control) {
		...
		const rtspStability = getIngestRTSPStability(inputs, control.process.reconnect);
		const processOptions = ['-loglevel', 'level+info', '-err_detect', rtspStability.decoderErrorMode];
		if (rtspStability.exitOnError) processOptions.push('-xerror');
		processOptions.push(...global);
		...
		const proc = {
			...
			reconnect: rtspStability.reconnect,
		...
		if (control.hls.dvr?.enabled) {
			const hours = Math.min(168, Math.max(1, parseInt(control.hls.dvr.hours, 10) || 4));
			const segmentDuration = Math.min(30, Math.max(1, parseInt(control.hls.segmentDuration, 10) || 2));
			control.hls.storage = 'diskfs';
			control.hls.cleanup = true;
			control.hls.segmentDuration = segmentDuration;
			control.hls.listSize = Math.ceil((hours * 3600) / segmentDuration);
			control.hls.dvr.hours = hours;
		}
		...
		if (control.hls.cleanup === true) {
			output.cleanup.push(
				...
				},
				{
					...
					// Real DVR segment lengths differ slightly from hls_time because
					// FFmpeg cuts at keyframes. An age limit based on the nominal
					// duration can remove files still referenced by the playlist.
					// Count-based and FFmpeg cleanup remain active.
					max_file_age_seconds: getHLSSegmentCleanupAge(control),
					purge_on_delete: true,
				},
			);
	...
	async UpdatePlayer(channelid) {
		...
		const aspectRatio = getPlayerAspectRatio(metadata, metadata.player.playback.autoAspectRatio === true);
		const playerHeight = aspectRatio === '4:3' ? 480 : 360;
		...
		const templateData = {
			...
			iframecode: this.GetPublicIframeCode(channelid, aspectRatio),
	...
	async UpdatePlayerConfig(channelid, metadata) {
		...
		const dvrEnabled = metadata.control.hls?.dvr?.enabled === true;
		const channel_id_h264 = metadata.control.preview?.enable && !dvrEnabled ? `${channelid}_h264` : channelid;

		const overlay = metadata.player.overlay;
		const positions = ['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-center', 'bottom-right'];
		const fonts = ['sans', 'sans-bold', 'serif', 'mono'];
		...
		const sanitizeLogo = (logo, index) => {
			const path = typeof logo.path === 'string' ? logo.path : '';
			const validPath = new RegExp(`^/channels/${channelid}/playeroverlaylogo${index}\\.(png|jpg|jpeg|webp)$`, 'i').test(path);
			return {
				enabled: logo.enabled === true && validPath,
				path: validPath ? path : '',
				position: positions.includes(logo.position) ? logo.position : index === 1 ? 'top-right' : 'top-left',
				width: number(logo.width, 16, 2000, 240),
				opacity: number(logo.opacity, 0.05, 1, 1),
				link: typeof logo.link === 'string' && /^https?:\/\//i.test(logo.link) ? logo.link : '',
			};
		};
		...
		const playerConfig = {
			...
			dvr: {
				enabled: dvrEnabled,
				hours: dvrEnabled ? metadata.control.hls.dvr.hours : 0,
				segmentDuration: dvrEnabled ? metadata.control.hls.segmentDuration : 0,
			},
			playback: {
				autoAspectRatio: metadata.player.playback.autoAspectRatio === true,
				aspectRatio: getPlayerAspectRatio(metadata, metadata.player.playback.autoAspectRatio === true),
				singleActiveStream: metadata.player.playback.singleActiveStream === true,
				sessionLimitEnabled: metadata.player.playback.sessionLimitEnabled === true,
				sessionLimitMinutes: 15,
			},
			overlay: {
				enabled: overlay.enabled === true,
				text: `channels/${channelid}/player-overlay.txt`,
				interval: number(overlay.interval, 15, 3600, 60),
				position: positions.includes(overlay.position) ? overlay.position : 'bottom-left',
				fontSize: number(overlay.fontSize, 8, 200, 32),
				font: fonts.includes(overlay.font) ? overlay.font : 'sans',
				fontColor: color(overlay.fontColor, '#ffffff'),
				box: overlay.box === true,
				boxColor: color(overlay.boxColor, '#000000'),
				boxOpacity: number(overlay.boxOpacity, 0, 1, 0.55),
				logo1: sanitizeLogo(overlay.logo1, 1),
				logo2: sanitizeLogo(overlay.logo2, 2),
			},
		};
	...
	async _getResources() {
		...
		const [activeSessions, activeSessionsError] = await this._call(this.api.ActiveSessions, ['hls']);
		const viewerCount = activeSessionsError === null ? countUniqueHLSViewers(activeSessions) : 0;
...
export { countUniqueHLSViewers, getHLSSegmentCleanupAge, getIngestRTSPStability, getPlayerAspectRatio, isDVRChannelId };
