...
export default function Edit({ restreamer = null }) {
	...
	const [$system, setSystem] = React.useState(M.getDefaultMetadata());
	const [$process, setProcess] = React.useState({});
	const [$ready, setReady] = React.useState(false);
	const [$deleteDialog, setDeleteDialog] = React.useState(false);
	const [$dvrPurgeDialog, setDVRPurgeDialog] = React.useState(false);
	...
	const load = async () => {
		...
		const system = await restreamer.GetMetadata();
		setSystem(system || M.getDefaultMetadata());
	...
	const handleDVRPurgeDialog = () => {
		setDVRPurgeDialog(!$dvrPurgeDialog);
	};

	const handleDVRPurge = async () => {
		setDVRPurgeDialog(false);
		setState((state) => ({ ...state, saving: true }));
		let result = { success: false, restartFailed: false };
		try {
			result = await restreamer.PurgeDVR(_channelid);
		} catch (error) {
		...
		if (result.success) {
			if (result.deletedFiles === 0) {
				notify.Dispatch('info', 'purge:dvr', i18n._(t`No stored DVR content was found.`));
			} else {
				notify.Dispatch('success', 'purge:dvr', i18n._(t`${result.deletedFiles} DVR files were deleted.`));
			}
			return;
		}

		if (result.restartFailed) {
			notify.Dispatch('error', 'purge:dvr-restart', i18n._(t`The DVR content was deleted, but the live stream could not be restarted.`));
		} else {
			notify.Dispatch('error', 'purge:dvr', i18n._(t`The DVR content could not be deleted.`));
		}
	};
	...
	return (
		<React.Fragment>
							...
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									<HLSControl
										...
										dvrMaxHours={$system.livechasing?.dvr?.maxHours || 4}
										onChange={handleControlChange('hls')}
										onPurgeDVR={handleDVRPurgeDialog}
										purging={$state.saving}
									/>
								</Grid>
								<Grid item xs={12}>
									<Divider />
			...
			<Dialog
				open={$dvrPurgeDialog}
				onClose={handleDVRPurgeDialog}
				title={<Trans>Delete DVR content for "{$data.meta.name}"?</Trans>}
				buttonsLeft={
					<Button variant="outlined" color="default" onClick={handleDVRPurgeDialog}>
						<Trans>Abort</Trans>
					</Button>
				}
				buttonsRight={
					<Button variant="outlined" color="secondary" onClick={handleDVRPurge}>
			...
			>
				<Typography>
					<Trans>
						The complete rewind history of this channel will be permanently deleted. If the channel is live, it will be interrupted briefly and
