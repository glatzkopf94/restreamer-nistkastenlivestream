...
test.each(['stable', 'strict'])('RTSP %s profile adds loss-tolerant buffering and forces TCP', (profile) => {
	const [input] = Network.func.createInputs(
		{
			...pullmatrix.settings,
			address: 'rtsp://127.0.0.1/live/stream',
			rtsp: { ...pullmatrix.settings.rtsp, transport: 'udp', stability: profile },
			general: { ...pullmatrix.settings.general, fflags: ['genpts', 'nobuffer'], thread_queue_size: 512 },
		},
		$config,
		$skills_ffmpeg8,
	);

	expect(input.livechasing).toEqual({ rtspStability: profile });
	...
	expect(input.options).not.toContain('+genpts+nobuffer');
});
