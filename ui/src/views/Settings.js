...
export default function Settings({ restreamer = null }) {
	...
	const [$dvr, setDVR] = React.useState({ maxHours: 4, minFreeGB: 20 });
	...
	const load = async () => {
		...
		const metadata = await restreamer.GetMetadata();
		if (metadata !== null) {
			setMetadata(metadata);
			setDVR({
				maxHours: metadata.livechasing?.dvr?.maxHours || 4,
				minFreeGB: metadata.livechasing?.dvr?.minFreeGB || 20,
			});
		}
	...
	const handleDVRChange = (what) => (event) => {
		let value = parseInt(event.target.value, 10);
		if (!Number.isFinite(value)) value = what === 'maxHours' ? 4 : 20;
		if (what === 'maxHours') value = Math.min(168, Math.max(1, value));
		if (what === 'minFreeGB') value = Math.min(500, Math.max(1, value));
		setDVR({ ...$dvr, [what]: value });
		setConfig({ ...$config, modified: true });
	};
	...
	const handleSave = async () => {
		...
		if (err !== null) {
			...
			const metadata = $metadata || (await restreamer.GetMetadata());
			if (metadata !== null) {
				metadata.livechasing = {
					...(metadata.livechasing || {}),
					dvr: { maxHours: $dvr.maxHours, minFreeGB: $dvr.minFreeGB },
				};
				const metadataSaved = await restreamer.SetMetadata(metadata);
				if (metadataSaved === false) {
					notify.Dispatch('error', 'save:dvr-settings', i18n._(t`DVR settings could not be saved.`));
					setSaving(false);
					return;
				}
				setMetadata(metadata);
	...
	const handlePurgeDVRDialog = () => {
		setDialogs({
			...$dialogs,
			purgeDVR: !$dialogs.purgeDVR,
		});
	};

	const handlePurgeAllDVR = async () => {
		setDialogs({ ...$dialogs, purgeDVR: false });
		setSaving(true);
		let result = { success: false, restartFailed: false };
		try {
			result = await restreamer.PurgeAllDVR();
		} catch (error) {
			result = { success: false, restartFailed: false };
		} finally {
			setSaving(false);
		}
		...
		if (result.success) {
			if (result.channelCount === 0) {
				notify.Dispatch('info', 'purge:dvr-all', i18n._(t`No DVR-enabled channels were found.`));
			} else if (result.deletedFiles === 0) {
				notify.Dispatch('info', 'purge:dvr-all', i18n._(t`No stored DVR content was found.`));
			} else {
				notify.Dispatch('success', 'purge:dvr-all', i18n._(t`${result.deletedFiles} DVR files from ${result.channelCount} channels were deleted.`));
			}
			return;
		}

		if (result.restartFailed) {
			notify.Dispatch('error', 'purge:dvr-all-restart', i18n._(t`The DVR content was deleted, but one or more live streams could not be restarted.`));
		} else {
			notify.Dispatch('error', 'purge:dvr-all', i18n._(t`The DVR content could not be deleted.`));
		}
	};
	...
	return (
		<React.Fragment>
				...
				<Grid container spacing={1}>
					<TabsVerticalGrid>
						<Tabs orientation="vertical" variant="scrollable" value={$tab} onChange={handleChangeTab}>
							...
							<ErrorTab className="tab" label="DVR" value="dvr" errors={false} />
						...
						<TabPanel value={$tab} index="dvr" className="panel">
							<Grid container spacing={2}>
								<Grid item xs={12}>
									<Typography variant="h2">DVR / Timeshift</Typography>
								</Grid>
								<Grid item xs={12}>
									<Typography variant="body1">
										<Trans>These limits apply to every channel for which DVR rewind is enabled.</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12} md={6}>
									<TextField
										type="number"
										label={<Trans>Maximum rewind time (hours)</Trans>}
										value={$dvr.maxHours}
								...
								<Grid item xs={12} md={6}>
									<TextField
										type="number"
										label={<Trans>Minimum free disk space (GB)</Trans>}
										value={$dvr.minFreeGB}
										onChange={handleDVRChange('minFreeGB')}
									/>
									<Typography variant="caption">
										<Trans>The oldest DVR segments are removed if free disk space falls below this reserve.</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<BoxText color="dark">
										<Typography>
											<Trans>At 4096 kbit/s, one enabled channel requires approximately 1.9 GB per hour.</Trans>
										</Typography>
									</BoxText>
								</Grid>
								<Grid item xs={12}>
								...
								<Grid item xs={12}>
									<Button variant="outlined" color="secondary" disabled={$saving} onClick={handlePurgeDVRDialog}>
										<Trans>Delete all DVR content</Trans>
									</Button>
									<Typography variant="caption" display="block">
										<Trans>Deletes the rewind history of every DVR-enabled channel without changing any settings.</Trans>
									</Typography>
								</Grid>
							</Grid>
						</TabPanel>
			...
			<Dialog
				...
				onClose={handlePurgeDVRDialog}
				buttonsLeft={
					<Button variant="outlined" color="default" onClick={handlePurgeDVRDialog}>
						<Trans>Abort</Trans>
					</Button>
				}
				buttonsRight={
					<Button variant="outlined" color="secondary" onClick={handlePurgeAllDVR}>
						<Trans>Delete all DVR content</Trans>
					</Button>
				}
			>
				<Typography variant="body1">
					<Trans>
						The complete rewind history of all DVR-enabled channels will be permanently deleted. Active live streams will be interrupted briefly and
						restarted automatically. Channel, player and DVR settings remain unchanged.
					</Trans>
				</Typography>
			</Dialog>
			<Dialog
