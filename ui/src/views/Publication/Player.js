...
export default function Edit({ restreamer = null }) {
	...
	const handleChange =
		...
		(event) => {
			...
			if (section === '') {
				...
				settings.playback[what] = !settings.playback[what];
			}
	...
	const handlePlayerOverlayChange = (overlay) => {
		if (timeout.current !== null) {
			clearTimeout(timeout.current);
			timeout.current = null;
		}
	...
	const handlePlayerOverlayStore = async (name, data) => {
		const path = await restreamer.UploadData(_channelid, name, data);
		setSaving(false);
		return path;
	};
	...
	const aspectRatio = restreamer.GetPlayerAspectRatio($metadata, $settings.playback.autoAspectRatio);
	const iframeCode = restreamer.GetPublicIframeCode(_channelid, aspectRatio);
				...
				<Grid container spacing={1} className={classes.gridContainer}>
					<Grid item xs={12}>
						<Grid container spacing={0} className={classes.playerL1}>
							<Grid item xs={12} className={classes.playerL2} style={{ paddingTop: aspectRatio === '4:3' ? '75%' : '56.25%' }}>
								{$state !== 'connected' ? (
				...
				<Grid container spacing={0}>
					<Grid item xs={12}>
						<TabsHorizontal value={$tab} onChange={handleChangeTab}>
							...
							<Tab className="tab" label={<Trans>Player overlay</Trans>} value="overlay" />
						...
						<TabPanel value={$tab} index="overlay">
							<PlayerOverlay
								settings={$settings.overlay}
								onChange={handlePlayerOverlayChange}
								onStore={handlePlayerOverlayStore}
						...
						<TabPanel value={$tab} index="playback">
							<Grid container spacing={2}>
								<Grid item xs={12}>
									...
									<Checkbox
										label={<Trans>Automatically detect 16:9 or 4:3</Trans>}
										checked={$settings.playback.autoAspectRatio}
										onChange={handleChange('autoAspectRatio', 'playback')}
									/>
									<Typography variant="body2" color="textSecondary">
										<Trans>
											The player uses the detected format before playback starts, avoiding black bars and a size change after pressing
											Play.
										</Trans>
									</Typography>
									<Checkbox
										label={<Trans>Allow only one active stream per viewer</Trans>}
										checked={$settings.playback.singleActiveStream}
										onChange={handleChange('singleActiveStream', 'playback')}
									/>
									<Typography variant="body2" color="textSecondary">
										<Trans>Starting another enabled player in the same browser deactivates the previous stream.</Trans>
									</Typography>
									<Checkbox
										label={<Trans>Require Play again after 15 minutes</Trans>}
										checked={$settings.playback.sessionLimitEnabled}
										onChange={handleChange('sessionLimitEnabled', 'playback')}
									/>
									<Typography variant="body2" color="textSecondary">
										<Trans>After 15 minutes of active playback, the stream is unloaded until the viewer explicitly continues.</Trans>
									</Typography>
								</Grid>
							</Grid>
						</TabPanel>
