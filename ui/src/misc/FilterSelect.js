...
export default function FilterSelect({
	...
	const handleFilterSettingsChange = (what) => (settings, graph, automatic) => {
		...
		for (let f of filterOrder) {
			...
			if (f !== 'drawtext' && filter.settings[f].graph.length !== 0) {
				graphs.push(filter.settings[f].graph);
			}
		}

		if (type === 'video' && filter.settings.drawtext) {
			graphs = [Filters.Video.Get('drawtext').composeGraph(filter.settings.drawtext.settings, graphs)];
		}

		filter.graph = graphs.filter((graph) => graph.length !== 0).join(',');
