# Vorhandene Entwicklungsinstanz auf 0.3.0-dev12 umstellen
...
docker run --rm \
  ...
  -v /root:/backup \
  alpine:3.24 \
  tar -C /source -czf /backup/restreamer-dev-config-before-0.3.0-dev12.tar.gz .
```
...
docker run --rm \
  ...
  -v /root:/backup \
  alpine:3.24 \
  tar -C /source -czf /backup/restreamer-dev-data-before-0.3.0-dev12.tar.gz .
```
...
RESTREAMER_IMAGE=ghcr.io/glatzkopf94/restreamer-nistkastenlivestream:0.3.0-dev12
...
```bash
cd /root/restreamer-nistkastenlivestream-0.3.0-dev12
...
Der neue Hintergrund, die Kennung `NKL 1.2 Beta` und die globale
