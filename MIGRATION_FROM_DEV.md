# Vorhandene Entwicklungsinstanz auf 0.3.0-dev11 umstellen
...
docker run --rm \
  ...
  -v /root:/backup \
  alpine:3.24 \
  tar -C /source -czf /backup/restreamer-dev-config-before-0.3.0-dev11.tar.gz .
```
...
docker run --rm \
  ...
  -v /root:/backup \
  alpine:3.24 \
  tar -C /source -czf /backup/restreamer-dev-data-before-0.3.0-dev11.tar.gz .
```
...
```dotenv
RESTREAMER_PROJECT_NAME=restreamer-livechasing
RESTREAMER_CONTAINER_NAME=restreamer-livechasing
RESTREAMER_IMAGE=ghcr.io/glatzkopf94/restreamer-nistkastenlivestream:0.3.0-dev11
...
```bash
cd /root/restreamer-nistkastenlivestream-0.3.0-dev11
...
Container heisst `restreamer-livechasing`; der Reverse-Proxy kann wegen des
...
Der neue Hintergrund, die Kennung `NKL 1.1 Beta` und die globale
