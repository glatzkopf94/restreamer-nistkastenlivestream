...
func migrateProcessConfig(logger log.Logger, config *app.Config, version string) (bool, error) {
	...
	// FFmpeg 7, 8, and 9 don't require process-option rewrites beyond the
	// existing 4 -> 5 migration. They are nevertheless valid targets and must
	// not prevent Core from starting with an already populated process store.
	if targetmajor > 9 {
	...
	if currentmajor >= 6 && currentmajor < targetmajor {
		// No process configuration changes are required for migrations from
		// FFmpeg 6 through FFmpeg 9.
