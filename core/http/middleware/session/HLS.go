...
var reViewerID = regexp.MustCompile(`^[A-Za-z0-9_-]{16,64}$`)
	...
	if isM3U8 {
		if !h.egressCollector.IsKnownSession(sessionID) {
			...
			if len(sessionID) != 0 {
				...
				viewerID := c.QueryParam("viewer")
				if reViewerID.MatchString(viewerID) {
					extra = "[viewer:" + viewerID + "] " + extra
				}
...
func (g *sessionRewriter) rewriteHLS(sessionID string, requestURL *url.URL) {
	...
	isMaster := false
	viewerID := requestURL.Query().Get("viewer")
