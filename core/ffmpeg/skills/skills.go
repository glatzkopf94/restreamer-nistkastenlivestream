...
func parseFilters(data []byte) []Filter {
	...
	// FFmpeg up to version 8 prints three capability columns (T/S/C), while
	// FFmpeg 9 removed the command-support column and prints only T/S. Accept
	// both layouts and flexible alignment whitespace.
	re := regexp.MustCompile(`^\s*[TSC.]{2,3}\s+([0-9A-Za-z_]+)\s+(?:\S+)\s+(.*)$`)
...
func parseFormats(data []byte) ffFormats {
	...
	// FFmpeg 9 aligns the format name with two spaces after the D/E flags
	// (e.g. " DE  rtsp"). Older versions used one. Accept both layouts.
